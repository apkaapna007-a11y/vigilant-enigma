...
export function MultiTopicInput({
  ...
  const addTopic = () => {
    ...
    onTopicsChange((current) => [
      ...current,
      { id, title: "", focusKeyword: "", status: "pending", progress: 0, createdAt: new Date().toISOString() },
    ]);
  ...
  const saveEdit = (id: string) => {
    updateTopic(id, { title: editTitle.trim(), focusKeyword: editKeyword.trim(), error: undefined });
    setEditingId(null);
  };
  ...
  const statusIcon: Record<Topic["status"], string> = { pending: "◦", processing: "⟳", completed: "✓", failed: "✕" };
    ...
    <section className="space-y-3" aria-labelledby="topics-heading">
      ...
      {topics.length === 0 ? (
        <div className="rounded-lg border border-dashed border-border bg-card/40 p-8 text-center text-muted-foreground">
          ...
          <p className="text-xs mt-1">Add a topic to start building your batch.</p>
        </div>
      ) : (
            ...
            >
              <div className="flex items-center gap-2 p-3">
                ...
                <div className={`flex items-center gap-1 px-2 py-1 rounded text-xs font-medium border shrink-0 ${statusClasses[topic.status]}`}>
                  <span aria-hidden="true">{statusIcon[topic.status]}</span><span className="capitalize">{topic.status}</span>
                </div>
                <button type="button" onClick={() => setExpandedId(expandedId === topic.id ? null : topic.id)} className="flex-1 min-w-0 text-left" disabled={disabled}>
                  ...
                  <p className="text-xs text-muted-foreground truncate">{topic.focusKeyword || "No focus keyword"}{topic.sourceFileName ? ` · ${topic.sourceFileName}` : ""}</p>
                </button>
              ...
              {expandedId === topic.id && (
                <div className="border-t border-border p-4 space-y-3 bg-muted/30">
                  {editingId === topic.id ? (
                    <>
                      <label className="block text-xs font-semibold">Article Topic<input value={editTitle} onChange={(event) => setEditTitle(event.target.value)} placeholder="e.g., Newborn Circumcision Care" className="input mt-1" maxLength={200} autoFocus /></label>
                      <label className="block text-xs font-semibold">Focus Keyword<input value={editKeyword} onChange={(event) => setEditKeyword(event.target.value)} placeholder="e.g., newborn circumcision care" className="input mt-1" maxLength={100} /></label>
                      <div className="flex gap-2"><button type="button" onClick={() => saveEdit(topic.id)} className="btn-primary flex-1 justify-center"><Check size={14} /> Save</button><button type="button" onClick={() => setEditingId(null)} className="btn-secondary flex-1 justify-center">Cancel</button></div>
                    </>
                  ) : (
                    <>
                      <div className="grid sm:grid-cols-2 gap-3 text-sm"><div><p className="text-xs font-semibold mb-1">Article Topic</p><p>{topic.title || <span className="text-muted-foreground">Not set</span>}</p></div><div><p className="text-xs font-semibold mb-1">Focus Keyword</p><p>{topic.focusKeyword || <span className="text-muted-foreground">Not set</span>}</p></div></div>
                      <button type="button" onClick={() => startEdit(topic)} className="w-full flex items-center justify-center gap-1.5 px-3 py-2 rounded bg-muted hover:bg-muted/80 text-sm font-medium"><Edit2 size={13} /> Edit Details</button>
                    </>
                  )}
                  {topic.sourceFileName && <div className="rounded bg-background border border-border p-2"><div className="flex items-center gap-2 text-xs"><FileText size={13} className="text-primary" /><span className="flex-1 truncate font-medium">{topic.sourceFileName}</span><button type="button" onClick={() => updateTopic(topic.id, { sourceFileName: undefined, sourceText: undefined })} className="p-0.5 hover:bg-muted rounded" disabled={disabled} aria-label="Remove source file"><X size={12} /></button></div><p className="text-[11px] text-muted-foreground mt-1">{topic.sourceText?.length.toLocaleString() || 0} characters loaded</p></div>}
                  <div onDrop={(event) => { event.preventDefault(); const file = event.dataTransfer.files?.[0]; if (file && !disabled) void handleFileForTopic(topic.id, file); }} onDragOver={(event) => event.preventDefault()} onClick={() => { if (!disabled) { pendingTopicIdRef.current = topic.id; fileInputRef.current?.click(); } }} className="border-2 border-dashed border-border rounded-lg p-3 text-center hover:border-primary/50 cursor-pointer bg-background"><Upload size={16} className="mx-auto mb-1 text-muted-foreground" /><p className="text-xs font-medium">Drop a source file or click to upload</p><p className="text-[11px] text-muted-foreground mt-0.5">.txt · .md · .docx · .pdf · .html</p></div>
                  {topic.error && <div className="flex gap-2 p-2 rounded bg-red-50 border border-red-200 text-red-700 text-xs"><AlertCircle size={14} className="shrink-0" /><span>{topic.error}</span></div>}
                  {topic.result && <div className="rounded bg-background border border-border p-2 max-h-32 overflow-y-auto"><p className="text-[11px] font-semibold text-muted-foreground mb-1">Result preview</p><p className="text-xs line-clamp-4">{topic.result}</p></div>}
                </div>
              )}
            </div>
          ))}
      ...
      <input ref={fileInputRef} type="file" accept=".txt,.md,.markdown,.docx,.pdf,.html,.htm" className="hidden" onChange={(event) => { const file = event.target.files?.[0]; const topicId = pendingTopicIdRef.current; if (file && topicId) void handleFileForTopic(topicId, file); pendingTopicIdRef.current = null; event.target.value = ""; }} disabled={disabled} />
    </section>
  );
}
